  ...
  const [storedValue, setStoredValue] = useState<T>(() => {
    try {
      ...
      if (Array.isArray(initialValue) && !Array.isArray(parsed)) {
        console.warn(`Mismatched type for key "${key}" in localStorage. Expected array, got ${typeof parsed}. Falling back to initial value.`);
        return initialValue;
      }

      // If the initial value is a non-null object (and not an array), ensure the parsed value is also a non-null object.
      if (typeof initialValue === 'object' && initialValue !== null && !Array.isArray(initialValue) && (typeof parsed !== 'object' || parsed === null || Array.isArray(parsed))) {
        console.warn(`Mismatched type for key "${key}" in localStorage. Expected object, got ${typeof parsed}. Falling back to initial value.`);
        return initialValue;
      }
