...
interface ChatMessageProps {
  ...
  onNavigateAlternate: (direction: 'prev' | 'next') => void;
}
...
const ChatMessage = React.memo(function ChatMessage({
  ...
  const hasAlternates = alternates && alternates.ids.length > 1;
  const canGoPrev = hasAlternates && alternates.activeIndex > 0;
  const canGoNext = hasAlternates && alternates.activeIndex < alternates.ids.length - 1;
  ...
  return (
    ...
    >
      ...
      >
        <div className="relative inline-block group/message-bubble">
          {/* Unified regeneration controls */}
          {!isUser && hasAlternates && (
            <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 flex items-center gap-2 px-2 py-1 bg-slate-900/70 backdrop-blur-sm border border-slate-700/50 rounded-full shadow-lg opacity-0 group-hover/message-bubble:opacity-100 transition-all duration-200 z-20">
              <button
                onClick={() => onNavigateAlternate('prev')}
                disabled={!canGoPrev}
                aria-label="Previous response"
                className="p-1 rounded-full text-slate-300 hover:bg-slate-700 hover:text-white disabled:opacity-30 disabled:cursor-not-allowed"
              >
                <Icon name="chevron-left" className="w-4 h-4" />
              </button>
              <span className="text-xs font-mono text-slate-400 select-none">
                {alternates.activeIndex + 1} / {alternates.ids.length}
              </span>
              <button
                onClick={() => onNavigateAlternate('next')}
                disabled={!canGoNext}
          ...
          >
            {!isEditing && !message.isThinking && !isSelectionModeActive && (
              <div
                ...
                >
                  ...
                  {!isUser && isLastMessage && (
                    <ActionButton icon="redo" label="Regenerate" onClick={onRegenerate} disabled={isLoading} className={isLoading ? 'animate-spin' : ''} />
                  )}
              </div>
            )}
            {isEditing ? (
