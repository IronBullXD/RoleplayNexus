...
const CustomCheckbox: React.FC<{
  ...
  return (
    <div className="flex items-center gap-3">
        ...
        {label && <label htmlFor={id} className="text-sm font-medium text-slate-300 cursor-pointer">{label}</label>}
    </div>
  );
};
...
const WorldItem: React.FC<{ 
    ...
    <div 
        ...
        className={`w-full flex items-center gap-2 p-2 pr-4 bg-slate-900/50 border hover:bg-slate-800/50 rounded-lg transition-all group cursor-pointer ${
            isSelected ? 'border-sky-500/80 bg-slate-800/50' : 'border-slate-800 hover:border-sky-500/50'
        }`}
    >
        <div className="flex items-center pl-2" onClick={e => e.stopPropagation()}>
             <CustomCheckbox id={`select-world-${world.id}`} checked={isSelected} onChange={onToggleSelect} />
        </div>
        ...
        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity shrink-0" onClick={e => e.stopPropagation()}>
            ...
            <button onClick={onExport} className="p-2 text-slate-400 hover:text-sky-400 hover:bg-slate-700/50 rounded-md" aria-label={`Export world: ${world.name}`}><Icon name="export" className="w-5 h-5" /></button>
...
const WorldsPage: React.FC<WorldsPageProps> = ({ onClose }) => {
    ...
    const handleExportWorld = (world: World) => {
        try {
            const filename = `world_${world.name.replace(/[^a-z0-9]/gi, '_').toLowerCase()}.json`;
                ...
                <div className="bg-slate-900 rounded-lg shadow-2xl w-full max-w-4xl flex flex-col border border-slate-700 h-[80vh] animate-slide-up" onClick={e => e.stopPropagation()}>
                    ...
                    <main className="flex-1 overflow-y-auto p-4 md:p-6 custom-scrollbar">
                        {worlds.length > 0 && (
                            <div className="flex items-center justify-between p-2 mb-2 border-b border-slate-800">
                                ...
                                {selectedWorldIds.size > 0 && <span className="text-sm text-slate-400 font-medium">{selectedWorldIds.size} of {worlds.length} selected</span>}
                            </div>
                        )}
                    ...
                    <footer className="p-4 border-t border-slate-800 flex justify-between items-center shrink-0">
                        <div className="flex items-center gap-2">
                            ...
                            <button onClick={() => fileInputRef.current?.click()} className="flex items-center gap-2 px-3 py-2 text-sm font-semibold text-slate-300 bg-slate-700/50 hover:bg-slate-700 rounded-md transition-colors border border-slate-600">
                            ...
                            >
                                ...
                                {selectedWorldIds.size > 0 ? `Export Selected (${selectedWorldIds.size})` : 'Export Selected'}
                            </button>
                        </div>
