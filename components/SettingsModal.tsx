...
const SectionButton: React.FC<{
  ...
  >
    {isActive && (
      <div className="absolute left-0 top-0 bottom-0 w-1 bg-sky-400 rounded-r-full"></div>
    )}
      ...
      >
        {Object.values(LLMProvider).map((p) => (
          <button
            ...
            className={`px-3 py-1.5 text-sm font-semibold rounded-md transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-sky-500 focus-visible:ring-offset-2 focus-visible:ring-offset-slate-900 ${
              settings.provider === p
      ...
      <div className="mt-4 space-y-4 border-t border-slate-700/50 pt-4">
        ...
        {!isGemini && (
          <FormField label="API Key" htmlFor={`${provider}-key`}>
            <input
              ...
              className="block w-full bg-slate-950 border-2 border-slate-700 rounded-lg shadow-sm focus:ring-sky-500 focus:border-sky-500 sm:text-sm p-3 placeholder:text-slate-600"
        ...
        <FormField label="Model Name" htmlFor={`${provider}-model`}>
          <input
            ...
            className="block w-full bg-slate-950 border-2 border-slate-700 rounded-lg shadow-sm focus:ring-sky-500 focus:border-sky-500 sm:text-sm p-3 placeholder:text-slate-600"
            placeholder={
              isGemini ? 'e.g., gemini-2.5-flash' : 'e.g., gryphe/mythomax-l2-13b'
            }
            required
          />
        </FormField>
  ...
  >
    ...
    >
      <textarea
        ...
        rows={8}
        className="block w-full bg-slate-950 border-2 border-slate-700 rounded-lg shadow-sm focus:ring-sky-500 focus:border-sky-500 sm:text-sm p-3 font-mono text-xs leading-relaxed custom-scrollbar"
      />
    </FormField>
    <FormField
    ...
    >
      <textarea
        ...
        rows={3}
        className="block w-full bg-slate-950 border-2 border-slate-700 rounded-lg shadow-sm focus:ring-sky-500 focus:border-sky-500 sm:text-sm p-3 font-mono text-xs leading-relaxed placeholder:text-slate-600 custom-scrollbar"
...
const SettingsModal: React.FC<SettingsModalProps> = ({ onClose }) => {
  ...
  return (
    ...
    >
      <div
        className="bg-slate-900 rounded-lg shadow-2xl w-full max-w-5xl flex flex-col border border-slate-700 h-[90vh] max-h-[800px] animate-slide-up"
      ...
      >
        ...
        >
          ...
          <footer className="p-4 border-t border-slate-800 flex justify-end space-x-3 shrink-0 bg-slate-900/95">
            ...
            <button
              type="submit"
              className="px-5 py-2 text-sm font-semibold text-white bg-sky-600 hover:bg-sky-500 rounded-lg transition-colors border border-sky-400/50 shadow-md shadow-sky-900/50"
            >
              Save Settings
            </button>
          </footer>
