...
import {
  ...
  getGroupChatCompletion,
  generateCharacterProfile as generateProfile,
  summarizeMessages,
...
interface AppActions {
  ...
  updateActiveSessionSettings: (
    update: Partial<ChatSession & GroupChatSession>,
  ) => void;
  handleSummarization: (
    session: ChatSession | GroupChatSession,
    ...
    (set, get) => ({
        ...
        set((state) => {
          ...
          if (hasGM) {
            characters = characters.map((c) =>
              c.id === GM_CHARACTER_ID ? GM_CHARACTER : c,
            );
          } else {
          ...
          if (customCharacters.length === 0) {
            const hasDefault = characters.some(
              (c) => c.id === DEFAULT_CHARACTER.id,
            );
            if (!hasDefault) {
      ...
      handleConfirm: () => {
        ...
        if (confirmationAction) {
          logger.uiEvent('Confirmation accepted', {
      ...
      handleCloseConfirmation: () => {
        const { confirmationAction } = get();
        if (confirmationAction)
          logger.uiEvent('Confirmation dismissed', {
            title: confirmationAction.title,
          });
        set({ isConfirmationModalOpen: false, confirmationAction: null });
      },
          ...
          () => {
            set((state) => {
              ...
              Object.keys(newGroupConversations).forEach((sid) => {
                ...
                if (session.characterIds.includes(id)) {
                  if (session.characterIds.length <= 2)
                    delete newGroupConversations[sid];
                  else
                    newGroupConversations[sid] = {
                      ...session,
                      characterIds: session.characterIds.filter(
                        (cid) => cid !== id,
                      ),
                    };
                }
              ...
              return {
                ...
                activeCharacterId: isDeletingActive
                  ? null
                  : state.activeCharacterId,
                activeSessionId: isDeletingActive
                  ? null
                  : state.activeSessionId,
                currentView: isDeletingActive
                  ? 'CHARACTER_SELECTION'
                  : state.currentView,
              };
            });
          ...
          () => {
            set((state) => {
              ...
              Object.keys(newConversations).forEach((charId) => {
                newConversations[charId] = newConversations[charId].map((s) =>
                  s.worldId === id ? { ...s, worldId: null } : s,
                );
              });
              const newGroupConversations = { ...state.groupConversations };
              Object.keys(newGroupConversations).forEach((sid) => {
                if (newGroupConversations[sid].worldId === id)
                  newGroupConversations[sid] = {
                    ...newGroupConversations[sid],
                    worldId: null,
                  };
              ...
              return {
                ...
                groupConversations: newGroupConversations,
                settings:
                  state.settings.worldId === id
                    ? { ...state.settings, worldId: null }
                    : state.settings,
              };
            });
          },
      ...
      logWorldEntryInteraction: (worldId, entryId) => {
        set((state) => {
          const interactions = JSON.parse(
            JSON.stringify(state.worldEntryInteractions),
          ); // deep copy
          if (!interactions[worldId]) {
            interactions[worldId] = {};
          }
          if (!interactions[worldId][entryId]) {
            interactions[worldId][entryId] = { viewCount: 0, lastViewed: 0 };
          }
      ...
      startChat: (id) => {
        ...
        if (existingSessions?.length) {
          const mostRecent = existingSessions.reduce((a, b) =>
        ...
        set((state) => ({
          conversations: {
            ...state.conversations,
            [id]: [...(state.conversations[id] || []), newSessionData],
          },
      ...
      selectSession: (characterId, sessionId) =>
        set({
      ...
      newSession: () => {
        ...
        set((state) => ({
          conversations: {
            ...state.conversations,
            [activeCharacterId]: [
              ...(state.conversations[activeCharacterId] || []),
              newSessionData,
            ],
          },
          activeSessionId: newSessionData.id,
      ...
      forkChat: (messageId) => {
        const { characters, activeCharacterId, conversations, activeSessionId } =
          get();
        const character = characters.find((c) => c.id === activeCharacterId);
        const session = conversations[activeCharacterId || '']?.find(
      ...
      deleteSession: (characterId, sessionId) => {
        ...
        const session = conversations[characterId]?.find(
          (s) => s.id === sessionId,
        );
        ...
        requestConfirmation(
          () =>
            set((state) => {
              ...
              newConvos[characterId] = (newConvos[characterId] || []).filter(
                (s) => s.id !== sessionId,
              );
              if (newConvos[characterId].length === 0)
                delete newConvos[characterId];
              return { conversations: newConvos };
            }),
          'Delete Chat History',
          `Are you sure you want to delete the chat "${session.title}" with "${char.name}"?`,
          'Delete',
          'danger',
        );
      },
      ...
      createGroupChat: (characterIds, scenario) => {
        ...
        const participating = characters.filter((c) =>
      ...
      deleteGroupSession: (sessionId) => {
        ...
        get().requestConfirmation(
          ...
          'Delete Group Chat',
          `Are you sure you want to delete the group chat "${session.title}"?`,
          'Delete',
          'danger',
        );
      },
      ...
      setTemperature: (temperature) =>
        get().updateActiveSessionSettings({ temperature }),
      setContextSize: (contextSize) =>
        get().updateActiveSessionSettings({ contextSize }),
      setMaxOutputTokens: (maxOutputTokens) =>
        get().updateActiveSessionSettings({ maxOutputTokens }),
      setMemoryEnabled: (memoryEnabled) =>
        get().updateActiveSessionSettings({ memoryEnabled }),

      updateActiveSessionSettings: (update) => {
        const { activeCharacterId, activeSessionId, activeGroupSessionId } =
          get();
        if (activeCharacterId && activeSessionId) {
          set((state) => ({
            conversations: {
              ...
              [activeCharacterId]: (
                ...
                s.id === activeSessionId ? { ...s, ...update } : s,
              ),
            },
          }));
          ...
          set((state) => {
            ...
            return {
              groupConversations: {
                ...state.groupConversations,
                [activeGroupSessionId]: { ...session, ...update },
              },
            };
          });
        }
      ...
      sendMessage: async (content) => {
        ...
        await get().runChatCompletion(char, session, [
          ...session.messages,
          userMessage,
        ]);
      },
      ...
      editMessage: async (messageId, newContent) => {
        ...
        if (msgToEdit.role === 'user') {
          ...
          set((state) => ({
            conversations: {
              ...state.conversations,
              [char.id]: (state.conversations[char.id] || []).map((s) =>
                s.id !== session.id
                  ? s
                  : {
                      ...s,
      ...
      continueGeneration: async () => {
        ...
        if (lastMessage?.role !== 'assistant') {
            logger.error("Tried to continue generation, but last message was not from assistant.", { lastMessage });
            return;
        }
      ...
      continueGroupGeneration: async () => {
        ...
        const session = groupConversations[activeGroupSessionId || ''];
        if (!session || isLoading) return;
        // For non-streaming, "continue" means generate the next turn.
      ...
      runGroupChatCompletion: async (session, messages) => {
        ...
        try {
          ...
          const turnActions = await getGroupChatCompletion({
            provider,
            apiKey,
            model,
            messages: messagesForApi,
            ...
            memorySummary: updatedSummary,
            activeCharacterNames: sessionChars.map((c) => c.name),
            interactionData:
          ...
          if (!turnActions || turnActions.length === 0) {
            throw new Error('AI returned an empty turn.');
          }

          const messageContent = turnActions
            .map((action) => {
              if (action.characterName.toLowerCase() === 'narrator') {
                return `*${action.content}*`;
              }
              return `**${action.characterName}:** ${action.content}`;
            })
            .join('\n\n');

          const firstSpeaker = turnActions.find(
            (a) => a.characterName.toLowerCase() !== 'narrator',
          );
          const firstSpeakerId = firstSpeaker
            ? characters.find((c) => c.name === firstSpeaker.characterName)?.id
            : undefined;
          ...
          finalMessage = {
            ...
            characterId: firstSpeakerId,
            timestamp: Date.now(),
          };
        } catch (err) {
          const msg = extractErrorMessage(err);
          set({ error: msg });
          logger.error('Group chat completion failed', {
            error: err,
            friendlyMessage: msg,
          });
          finalMessage = {
